...
function toLocalizedRecord(
    ...
    if (typeof value === 'string') {
        return Object.fromEntries(locales.map((locale) => [locale, value])) as Record<
            string,
            string
        >
    }
    ...
    return Object.fromEntries(
        locales.map((locale) => [locale, value[locale] ?? fallback])
...
export async function loadMockAppConfig(): Promise<TSubscriptionPageRawConfig> {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to load mock app-config from ${MOCK_SOURCE_URL}: ${response.status}`
            )
        }
